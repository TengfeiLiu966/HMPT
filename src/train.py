...
class sentence_genaration(customizedModule):
    def __init__(self,):
        ...
        self.sentence_num = 100

    def forward(self, word_feature,sentence_mask,device):
        sentence_feature = []
        for i in range(word_feature.size(0)):
            nn = 0
            sentence_feature_s = []
            index = torch.argmax(sentence_mask[i, :, :].view(-1), dim=0)
            bb = sentence_mask[i, :, :].view(-1)[index].type(torch.int32)
            for j in range(bb):
                mid_mask = (sentence_mask.view(word_feature.size(0), -1)[i, :] == j + 1).unsqueeze(-1).repeat(1, 768)
                sentence_feature_s.append(torch.max(word_feature[i, :, :][mid_mask].view(-1, 768), dim=0)[0].unsqueeze(0))
            if len(sentence_feature_s) < self.sentence_num:
                sentence_feature_s.append(torch.tensor((self.sentence_num - len(sentence_feature_s)) * [[0] * 768]).to(device))
            else:
                sentence_feature_s = sentence_feature_s[:self.sentence_num]
...
class ClassifyModel(customizedModule):
    def __init__(self,pretrained_model_name_or_path,num_labels,args,Encoder2,device,is_lock = False):
        ...
        self.missing_img_prompt_ss = nn.Parameter(torch.zeros(4, 16, 768))
        self.missing_img_prompt_se = nn.Parameter(torch.zeros(4, 16, 768))
        self.missing_img_prompt_ee = nn.Parameter(torch.zeros(4, 16, 768))
        if is_lock:
    ...
    def forward(self, input_ids,image1,sentence_mask,token_type_ids = None,attention_mask = None,label = None,):
        global_text = self.bert(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)   #4*256*768
        #文本特征********************************************************************************************************************
        #单词特征
        local_text1 = global_text.last_hidden_state[:, 1:-1, :]  # 获取段中所有单词的特征
        word_feature = torch.reshape(local_text1, (-1,self.args.max_sec_num*(self.args.max_seq_length-2), 768))   #4*2048*768
        # # 句子特征
        sentence_feature = self.sentence_genaration(word_feature,sentence_mask,self.device)
        image1 = self.g_W1(image1)  # 做了一次映射
        image1,att_image = self.Encoder2(image1, mask=None,mask1=None,mask2=None)  # 2*10*768
        image_0 = torch.max(image1, dim=1)[0]

        global_ouput_1,global_ouput_3 = self.SectionOne(global_text.pooler_output.view(-1, self.args.max_sec_num, 768), image1,self.missing_img_prompt_ss,self.missing_img_prompt_se)
        global_ouput_2,global_ouput_4 = self.SentenceFour(sentence_feature, image1,image_0,self.missing_img_prompt_ee,self.missing_img_prompt_se)

        u33 = torch.cat([global_ouput_1.unsqueeze(1), global_ouput_2.unsqueeze(1), global_ouput_3.unsqueeze(1), global_ouput_4.unsqueeze(1)], dim=1)
