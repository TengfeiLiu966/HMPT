...
class ClassifyModel(customizedModule):
    def __init__(self,pretrained_model_name_or_path,num_labels,args,Encoder2,device,is_lock = False):
        ...
        self.prompt_ss = nn.Parameter(torch.zeros(4, 16, 768))
        self.prompt_se = nn.Parameter(torch.zeros(4, 16, 768))
        self.prompt_ee = nn.Parameter(torch.zeros(4, 16, 768))
        if is_lock:
    ...
    def forward(self, input_ids,image1,sentence_mask,token_type_ids = None,attention_mask = None,label = None,):
        ...
        word_feature = torch.reshape(local_text1, (-1,8*(self.args.max_seq_length-2), 768))   #4*2048*768
        # # 句子特征
        sentence_feature = self.sentence_genaration(word_feature,sentence_mask,self.device)
        image1 = self.g_W1(image1)  # 做了一次映射
        image1 = self.Encoder2(image1, mask=None)  # 2*10*768
        image_0 = torch.max(image1, dim=1)[0]

        global_ouput_1,global_ouput_3 = self.SectionOne(global_text.view(-1, 8, 768), image1, self.prompt_ss, self.prompt_se)
        global_ouput_2,global_ouput_4 = self.SentenceFour(sentence_feature, image1, image_0, self.prompt_ee, self.prompt_se)
